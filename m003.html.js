...
var KEY="deliverySystemV45_", COMPLETE_KEY="DeliverySystemV72_businessCompleted", STORE_KEY="m003WorkData";
...
function load(){var d=parse(STORE_KEY,{});id("workText").value=d.workText||"";id("noticeText").value=d.noticeText||"";}
function saveWork(silent){save(STORE_KEY,{customerCode:"M003",customerName:"株式会社カーゴジャパンアドバンスト",workText:id("workText").value||"",noticeText:id("noticeText").value||"",savedAt:new Date().toLocaleString("ja-JP")});localStorage.setItem(COMPLETE_KEY,"0");if(!silent)alert("保存しました。");}
...
(function(){
  var KEY="deliverySystemV45_", CUSTOMER="M003";
  function setActive(){
    try{
